...
| Antigravity skills install to `~/.gemini/config/skills` | That home is migrated. Antigravity replaced `~/.gemini/antigravity-cli/skills` with a compatibility symlink and left a `.migrated` marker; the installer follows the vendor to the real directory instead of writing through the link. | Nothing to do. An unmigrated home still installs to `~/.gemini/antigravity-cli/skills`. A link pointing anywhere else is not followed and the target is skipped instead. |
| Antigravity logs `invalid frontmatter format` for a rule file | Antigravity rejects a plugin rule with no YAML frontmatter and loads nothing from it. Rule files this installer wrote before frontmatter was added are still on disk. | Reinstall the `instruction-doc` artifacts. `verify` reports this as `antigravity-rule-frontmatter`. |
| Install reports `relocated_records` | An agent moved a managed directory and left a link behind, so records still named the old path. The install repointed them at the artifacts they describe. | Nothing to do. Only the records moved; no file was read, written, or removed, and each record keeps its original uninstall origin. |
