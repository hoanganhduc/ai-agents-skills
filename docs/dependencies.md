...
| `kaggle` | The new single Kaggle API token is configured outside this repo and projected through the guarded KAGGLE_API_TOKEN environment variable at runtime; kagglehub validates it and the kaggle CLI authenticates kernel ops with it. It is never pathname-read, written to argv, logs, a legacy kaggle.json, or a kernel, and the legacy KAGGLE_USERNAME + KAGGLE_KEY pair is not used. ToS: use for modest, legitimate research workloads only. | `kaggle-research-compute` |
