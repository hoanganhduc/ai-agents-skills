...
The current manifest contains 38 installable skills; the matrix below describes the target/surface support contract that those generated skill and artifact plans use.
...
| `antigravity` | `skill-file` | `supported` | `reference-adapter` | Antigravity global flat Markdown skill file | `official-docs` | Auto mode writes ~/.gemini/antigravity-cli/skills/<skill>.md adapters, matching the documented global skill layout. |
