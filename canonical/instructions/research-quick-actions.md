...
| Scripted force-loop (default, all OS) | `arl-scripted-force-loop` + runtime `force-loop/` (`bootstrap` / `start`; enforce + hard + notify) |
| Opt formal tools into ARL / headless drive | `autonomous-loop-formal-policy` + force-loop `--profile formal` (thin sample: `sample-arl-headless-driver-with-formal`) |
