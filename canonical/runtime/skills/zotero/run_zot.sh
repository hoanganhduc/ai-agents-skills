...
# Capture only this wrapper's authorities with shell builtins. Remove every
# ambient credential and startup hook before resolving a path or spawning a
# process, then restore the exact Zotero projection only to the bound child.
...
python_real="$PYTHON"
select_unused_fd AAS_ZOTERO_PYTHON_FD || exit 127
eval "exec ${AAS_ZOTERO_PYTHON_FD}<\"\$PYTHON\"" || exit 127
if [ -e "/proc/self/fd/$AAS_ZOTERO_PYTHON_FD" ]; then
  PYTHON="/proc/self/fd/$AAS_ZOTERO_PYTHON_FD"
  [ "$PYTHON" -ef "$python_real" ] || exit 127
elif [ -e "/dev/fd/$AAS_ZOTERO_PYTHON_FD" ]; then
  # Darwin's fdesc node is not executable; use the fixed attested path.
  /usr/bin/python3 -I -c 'import os,sys; a=os.fstat(int(sys.argv[1])); b=os.stat("/usr/bin/python3"); sys.exit((a.st_dev,a.st_ino)!=(b.st_dev,b.st_ino))' "$AAS_ZOTERO_PYTHON_FD" || exit 127
  PYTHON="$python_real"
else
...
[ -n "$zotero_api_key" ] && export ZOTERO_API_KEY="$zotero_api_key"
[ -n "$webdav_password" ] && export WEBDAV_PASSWORD="$webdav_password"
[ -n "$gdrive_credentials" ] && export GDRIVE_CREDENTIALS="$gdrive_credentials"
[ -n "$semantic_scholar_key" ] && export SEMANTIC_SCHOLAR_API_KEY="$semantic_scholar_key"
...
exec -a "$python_argv0" "$PYTHON" -I -c "$secure_loader" "$SCRIPT_DIR/zot.py" "$@"
