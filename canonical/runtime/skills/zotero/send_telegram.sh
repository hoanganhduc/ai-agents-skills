...
# TemporaryFile is anonymous on POSIX and stays alive until exec; only stdin
# receives the checked script bytes. The separate FD preserves the request.
secure_shell_loader='import os,stat,sys,tempfile; p=os.path.abspath(sys.argv[1]); q=os.stat(p,follow_symlinks=False); f=os.open(p,os.O_RDONLY|getattr(os,"O_NOFOLLOW",0)|getattr(os,"O_NONBLOCK",0)); b=os.fstat(f); ok=stat.S_ISREG(b.st_mode) and b.st_nlink==1 and b.st_uid in {0,os.geteuid()} and not (stat.S_IMODE(b.st_mode)&0o022) and (q.st_dev,q.st_ino)==(b.st_dev,b.st_ino) and b.st_size<=1048576; ok or (_ for _ in ()).throw(RuntimeError("file-delivery entrypoint is unavailable or untrusted")); d=b""; rem=b.st_size; exec("while rem:\n c=os.read(f,min(65536,rem))\n c or (_ for _ in ()).throw(RuntimeError(\"file-delivery entrypoint was truncated\"))\n d+=c; rem-=len(c)"); a=os.fstat(f); (b.st_dev,b.st_ino,b.st_size,b.st_mtime_ns,b.st_ctime_ns,b.st_nlink)==(a.st_dev,a.st_ino,a.st_size,a.st_mtime_ns,a.st_ctime_ns,a.st_nlink) or (_ for _ in ()).throw(RuntimeError("file-delivery entrypoint changed while reading")); request_fd=os.dup(0); os.set_inheritable(request_fd,True); storage=None if hasattr(os,"memfd_create") else tempfile.TemporaryFile(); m=os.memfd_create("aas-send-file",0) if storage is None else storage.fileno(); os.write(m,d)==len(d) or (_ for _ in ()).throw(RuntimeError("short protected script write")); os.lseek(m,0,os.SEEK_SET); os.dup2(m,0); allowed=("HOME","LANG","LC_ALL","TZ","AAS_RUNTIME_WORKSPACE","OPENCLAW_WORKSPACE","AAS_FILE_DELIVERY_SECRETS_FILE","AAS_ZOTERO_DELIVERY_DIR"); env={k:os.environ[k] for k in allowed if os.environ.get(k)}; env.update({"PATH":"/usr/bin:/bin","AAS_FILE_DELIVERY_REQUEST_FD":str(request_fd)}); os.execve("/bin/bash",["/bin/bash","-p","-s","--"],env)'
