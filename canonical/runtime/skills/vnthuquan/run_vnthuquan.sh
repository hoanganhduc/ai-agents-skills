...
unset GDRIVE_CREDENTIALS CALIBRE_GDRIVE_FOLDER_ID ZOTERO_API_KEY WEBDAV_PASSWORD
...
RUNTIME_ROOT="$(cd -- "$SCRIPT_DIR/../../.." && builtin pwd -P)"
...
export AAS_RUNTIME_ROOT="$RUNTIME_ROOT"
...
export VNTHUQUAN_TARGET="${VNTHUQUAN_TARGET:-posix-ai-agents-skills}"
export VNTHUQUAN_ASSISTANT_HOME="$RUNTIME_ROOT"
# Credential pointers can reach only this fixed, managed nested runner and its
# exact Calibre command; caller-provided runner/script overrides are ignored.
export VNTHUQUAN_CALIBRE_RUNNER="$RUNTIME_ROOT/run_skill.sh"
export VNTHUQUAN_CALIBRE_SCRIPT="skills/calibre/run_cal.sh"
export VNTHUQUAN_CALIBRE_CACHE_PATH="$RUNTIME_WORKSPACE/data/calibre/cache/library.json"
...
exec "$PYTHON" -I -c "$secure_loader" "$SCRIPT_DIR/vnthuquan_wrapper.py" "$@"
