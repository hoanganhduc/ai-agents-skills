...
# No generic authority, ambient channel credential, or secret pointer may reach
# path resolution or interpreter attestation.  The dedicated pointer is restored
# only after the system interpreter has been descriptor-bound.
unset REMOTE_BRIDGE_SECRETS_FILE
...
unset ZULIP_ORG_URL ZULIP_SITE ZULIP_EMAIL ZULIP_API_KEY ZULIP_CONTROL_STREAM ZULIP_TOPIC_PREFIX
unset ZULIP_ALLOWED_USER_IDS TELEGRAM_BOT_TOKEN TELEGRAM_ALLOWED_CHAT_IDS
unset TELEGRAM_ALLOWED_USER_IDS TELEGRAM_MODE
...
SCRIPT="$SCRIPT_DIR/remote_bridge.py"
...
if [ -n "$configured_python" ] && [ ! "$configured_python" -ef "$PYTHON" ] 2>/dev/null; then
  ...
  # CSR currently publishes this selector before a qualified closure exists.
  # remote_bridge.py is stdlib-only, so retain integration compatibility while
  # executing the already-attested system Python.  The caller-selected closure
