...
if ($env:OPENGAUSS_RUN_ARG_COUNT -match '^\d+$') {
    $envArgs = New-Object System.Collections.Generic.List[string]
    for ($i = 0; $i -lt [int]$env:OPENGAUSS_RUN_ARG_COUNT; $i++) {
        $envArgs.Add([Environment]::GetEnvironmentVariable("OPENGAUSS_RUN_ARG_$i"))
    }
    $SkillArgs = $envArgs.ToArray()
}

$script = Join-Path $PSScriptRoot "opengauss.py"
