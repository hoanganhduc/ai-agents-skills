...
# Hetzner is a distinct compute authority.  Scrub the Kaggle lane even when
# an ambient HCLOUD_TOKEN selects the branch below; otherwise that branch
# previously retained unrelated compute credentials in the child process.
...
if ($env:AAS_COMPUTE_SECRETS_FILE -or $env:HCLOUD_TOKEN) {
    ...
    if (-not $SecretLoader) {
        [Console]::Error.WriteLine(
            "Managed compute secret loader is unavailable."
        )
        exit 127
    }
...
$env:AAS_RUNTIME_SCRIPT = Join-Path $ScriptDir "hetzner_research_compute.py"
