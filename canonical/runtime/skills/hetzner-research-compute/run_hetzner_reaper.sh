...
resolve_python() {
  ...
  if [ -z "$resolved" ] || [ ! -f "$resolved" ] || [ ! -x "$resolved" ]; then
    printf 'hetzner reaper requires a usable Python runtime\n' >&2
    return 127
  fi
  case "$resolved" in
    /proc/self/fd/*|/dev/fd/*)
...
if { [ -n "$compute_pointer" ] || [ -n "$hcloud_token" ] || [ -n "$hcloud_ssh_keys" ]; } &&
  ...
  printf 'Hetzner reaper launch could not bind the trusted Python inode.\n' >&2
  exit 127
fi
...
command=("$PYTHON" -I "$ROOT/hetzner_reaper.py" "$@")
