...
| Drive exits `runtime_error`: "transaction journal quarantined" | Inspect `manifest.json` under the quarantine directory the message names (`$LOOP/.goal_focus_transactions_quarantine/<id>-<timestamp>-<random>/`) against the live targets, then re-run `start`; the loop is not wedged |
