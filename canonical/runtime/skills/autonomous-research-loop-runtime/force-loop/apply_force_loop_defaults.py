...
def write_host_env_defaults(run_dir: Path, profile: str, policy_file: Path) -> Path:
...
def apply_defaults(
    ...
    *,
    ...
    legacy_policy = run_dir / "driver" / "force_loop.env"
    if legacy_policy.is_symlink():
        raise ValueError("refusing symlinked legacy loop-local force_loop.env")
    if legacy_policy.is_file():
        backup_dir = run_dir / "driver" / "force_loop_pin_backups"
        _backup(legacy_policy, backup_dir)
    ...
    env_path = write_host_env_defaults(run_dir, profile, policy_file)
