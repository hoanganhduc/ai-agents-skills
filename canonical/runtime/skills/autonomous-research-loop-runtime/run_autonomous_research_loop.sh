...
resolve_python() {
  ...
  if [ "$credential_present" -eq 1 ]; then
    case "$configured" in
      ""|/*) ;;
      *)
        printf 'secret load requires a trusted already-resolved Python runtime\n' >&2
        return 127
        ;;
    esac
    resolved=""
    ...
    if [ -z "$resolved" ]; then
      printf 'secret load requires a trusted managed or system Python runtime\n' >&2
      return 127
    fi
  ...
  if [ -z "$resolved" ] || [ ! -f "$resolved" ] || [ ! -x "$resolved" ]; then
    printf 'autonomous-research-loop-runtime requires a usable Python runtime\n' >&2
    return 127
  fi
  case "$resolved" in
    /proc/self/fd/*|/dev/fd/*)
...
resolve_secret_loader() {
  ...
  for candidate in \
    "$SCRIPT_DIR/../../../load_secret_env.py" \
    "$SCRIPT_DIR/../../runners/load_secret_env.py"; do
...
if [ "${1:-}" = "drive" ] && {
  ...
  if [ -z "$secret_loader" ]; then
    echo "autonomous-research-loop-runtime managed secret loader is unavailable" >&2
    exit 127
  fi
  if [ -n "${AAS_COMPUTE_SECRETS_FILE:-}" ]; then
    command=(
      ...
      --retain-env ANTHROPIC_API_KEY
      --retain-env ANTHROPIC_AUTH_TOKEN
      --retain-env CLAUDE_API_KEY
      --retain-env CLAUDE_CODE_OAUTH_TOKEN
      --retain-env COPILOT_GITHUB_TOKEN
      --retain-env COPILOT_PROVIDER_API_KEY
      --retain-env COPILOT_PROVIDER_BEARER_TOKEN
      --retain-env DEEPSEEK_API_KEY
      --retain-env GEMINI_API_KEY
      --retain-env GH_TOKEN
      --retain-env GITHUB_TOKEN
      --retain-env GOOGLE_API_KEY
      --retain-env GROK_API_KEY
      --retain-env KIMI_API_KEY
      --retain-env MOONSHOT_API_KEY
      --retain-env OPENAI_API_KEY
      --retain-env OPENCODE_API_KEY
      --retain-env XAI_API_KEY
      -- "${command[@]}"
    )
  fi
  if [ -n "${AAS_PROVIDER_SECRETS_FILE:-}" ]; then
    command=(
      ...
      --allow-key CLAUDE_API_KEY
      --allow-key CLAUDE_CODE_OAUTH_TOKEN
      --allow-key COPILOT_GITHUB_TOKEN
      --allow-key COPILOT_PROVIDER_API_KEY
      --allow-key COPILOT_PROVIDER_BEARER_TOKEN
      --allow-key DEEPSEEK_API_KEY
      --allow-key GEMINI_API_KEY
      --allow-key GH_TOKEN
      --allow-key GITHUB_TOKEN
      --allow-key GOOGLE_API_KEY
      --allow-key GROK_API_KEY
      --allow-key KIMI_API_KEY
      --allow-key MOONSHOT_API_KEY
      --allow-key OPENAI_API_KEY
      --allow-key OPENCODE_API_KEY
      --allow-key XAI_API_KEY
      --export-key ANTHROPIC_API_KEY
      --export-key ANTHROPIC_AUTH_TOKEN
      --export-key CLAUDE_API_KEY
      --export-key CLAUDE_CODE_OAUTH_TOKEN
      --export-key COPILOT_GITHUB_TOKEN
      --export-key COPILOT_PROVIDER_API_KEY
      --export-key COPILOT_PROVIDER_BEARER_TOKEN
      --export-key DEEPSEEK_API_KEY
      --export-key GEMINI_API_KEY
      --export-key GH_TOKEN
      --export-key GITHUB_TOKEN
      --export-key GOOGLE_API_KEY
      --export-key GROK_API_KEY
      --export-key KIMI_API_KEY
      --export-key MOONSHOT_API_KEY
      --export-key OPENAI_API_KEY
      --export-key OPENCODE_API_KEY
      --export-key XAI_API_KEY
      --retain-env AAS_COMPUTE_SECRETS_FILE
      --retain-env PYTHONPATH
      -- "${command[@]}"
    )
  fi
