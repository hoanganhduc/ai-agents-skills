...
if ($CredentialBearingLaunch -and -not $env:AAS_COMPUTE_SECRETS_FILE) {
    ...
    if ($LASTEXITCODE -ne 0 -or -not $ResolvedPython) {
        [Console]::Error.WriteLine(
            "Credential-bearing Modal launch requires a trusted resolved Python runtime."
        )
        exit 127
    }
...
$env:AAS_RUNTIME_SCRIPT = Join-Path $ScriptDir "modal_research_compute.py"
