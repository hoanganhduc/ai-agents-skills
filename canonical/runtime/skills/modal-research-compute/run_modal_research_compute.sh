...
unset AAS_COMPUTE_SECRETS_FILE MODAL_TOKEN_ID MODAL_TOKEN_SECRET
...
resolve_python() {
  ...
  if [ -n "$compute_pointer" ] ||
    [ -n "$modal_token_id" ] || [ -n "$modal_token_secret" ]; then
  ...
  if [ "$credential_present" -eq 1 ]; then
    case "$configured" in
      ""|/*) ;;
      *)
        printf 'credential-bearing Modal launch requires a trusted already-resolved Python runtime\n' >&2
        return 127
        ;;
    esac
    resolved=""
    ...
    if [ -z "$resolved" ]; then
      printf 'credential-bearing Modal launch requires a trusted managed or system Python runtime\n' >&2
      return 127
    fi
  ...
  if [ -z "$resolved" ] || [ ! -f "$resolved" ] || [ ! -x "$resolved" ]; then
    printf 'modal-research-compute requires a usable Python runtime\n' >&2
    return 127
  fi
  case "$resolved" in
    /proc/self/fd/*|/dev/fd/*)
...
command=("$PYTHON" -I "$ROOT/modal_research_compute.py" "$@")
if [ -n "$compute_pointer" ]; then
  ...
  [ -n "$modal_token_id" ] && export MODAL_TOKEN_ID="$modal_token_id"
  [ -n "$modal_token_secret" ] && export MODAL_TOKEN_SECRET="$modal_token_secret"
  command=(
