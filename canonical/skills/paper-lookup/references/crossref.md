...
None required. Add `mailto=<EMAIL>` to get into the **polite pool** (2x faster rate limits).
...
```
GET /works?query={text}&rows={n}&mailto=<EMAIL>
```
...
**Example:**
```
https://api.crossref.org/works?query=CRISPR+gene+therapy&filter=from-pub-date:2024-01-01,type:journal-article,has-abstract:true&rows=5&sort=published&order=desc&mailto=<EMAIL>
```
...
**Example:**
```
https://api.crossref.org/works/10.1038%2Fnature12373?mailto=<EMAIL>
```
